...
def create_target_variable(data: pd.DataFrame, params: Dict[str, Any]) -> pd.DataFrame:
    ...
    data_copy[params["target_col"]] = (
        data_copy[params["original_target_col"]] > params["salary_threshold"]
    ).astype(int)
    data_copy = data_copy.drop(columns=[params["original_target_col"]])
    logger.info(
        f"Variable objetivo '{params['target_col']}' creada. Distribución:\n{data_copy[params['target_col']].value_counts(normalize=True)}"
    )
    return data_copy
...
def _get_model_instance(model_name: str, params: Dict[str, Any]):
    ...
    model_map = {
        "LogisticRegression": LogisticRegression(
            random_state=params.get("random_state"), max_iter=1000
        ),
...
def train_classifier_with_grid_search(
    ...
    pipeline = ImbPipeline(
        [("smote", SMOTE(random_state=params.get("random_state"))), ("model", model)]
    )
...
def report_and_select_best_classifier(
    ...
    for model_name, model in models.items():
        ...
        confusion_matrices[model_name] = (
            cm.tolist()
        )  # Guardar como lista para serialización
    ...
    logger.info(
        f"Mejor modelo seleccionado: {type(best_model.named_steps['model']).__name__} (F1-Score: {best_f1:.4f})"
    )
