...
def cargar_datos_raw_chile() -> pd.DataFrame:
    ...
    for chunk in pd.read_csv('data/01_raw/stackoverflow_2023/stack_overflow_survey_results_public.csv', 
    ...
    logger.info(f"Stack Overflow 2023 - Chile: {len(chile_so_2023)} registros de {total_processed:,} totales")
...
def generar_predicciones_chile(
    df_chile_procesado: pd.DataFrame,
    modelo_regresion: Any,
    modelo_clasificacion: Any
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if hasattr(modelo_regresion, 'feature_names_in_'):
            ...
            expected_features = df_predicciones.select_dtypes(include=[np.number]).columns.tolist()
    ...
    try:
        ...
        if X_features is None:
            if hasattr(modelo_clasificacion, 'feature_names_in_'):
                ...
                expected_features_clf = df_predicciones.select_dtypes(include=[np.number]).columns.tolist()
        ...
        logger.info(f"Predicciones de experiencia generadas: {len(predicciones_experiencia)}")
...
def analisis_comparativo_chile_global(
    ...
    if salary_col_global and 'salario_predicho' in df_predicciones_chile.columns:
        ...
        reporte['salarios'] = {
            ...
            'comparacion': {
                ...
                'brecha_mediana': float(salarios_global.median() - salarios_chile.median()),
                'ratio_chile_global': float(salarios_chile.mean() / salarios_global.mean())
            }
        }
        
        logger.info(f"Salario medio Chile: {reporte['salarios']['chile']['media']:.2f}")
        logger.info(f"Salario medio Global: {reporte['salarios']['global']['media']:.2f}")
    ...
    if 'experiencia_predicha' in df_predicciones_chile.columns:
        ...
        experiencia_data = {
            'chile_distribucion': exp_chile.to_dict()
        }
    ...
    try:
        if hasattr(modelo_regresion, 'feature_importances_'):
            ...
            feature_names = df_procesado_global.select_dtypes(include=[np.number]).columns.tolist()
    ...
    if 'salario_real' in df_predicciones_chile.columns and 'salario_predicho' in df_predicciones_chile.columns:
        ...
        logger.info(f"R² en Chile: {reporte['metricas_prediccion_chile']['r2_score']:.4f}")
...
def exportar_reporte_comparativo(
    reporte_comparativo: Dict[str, Any]
) -> pd.DataFrame:
    """
