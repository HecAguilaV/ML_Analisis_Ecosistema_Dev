...
def analizar_distribucion_global(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    salary_cols = [
        col for col in df.columns if "salary" in col.lower() or "comp" in col.lower()
    ]
    ...
    stats["tipos_datos"] = df.dtypes.value_counts().to_dict()
...
def generar_reporte_preliminar(
    df: pd.DataFrame, stats_globales: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    reporte["porcentaje_completitud"] = float(
